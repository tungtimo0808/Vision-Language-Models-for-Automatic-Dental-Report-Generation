...
from frcnn_common import build_faster_rcnn_model, coco_id_to_fdi_map, load_yaml, resolve_device
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("faster_rcnn_fdi/outputs/best.pth"),
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("faster_rcnn_fdi/outputs/prediction.json"),
        help="Output predictions json",
    )
    parser.add_argument(
        "--output-visual",
        type=Path,
        default=Path("faster_rcnn_fdi/outputs/prediction_vis.jpg"),
...
def main() -> None:
    ...
    cat_to_fdi = coco_id_to_fdi_map(categories)
    ...
    for score, label, box in zip(scores, labels, boxes):
        pred_rows.append(
            {
                "tooth_id": int(cat_to_fdi.get(int(label), int(label))),
